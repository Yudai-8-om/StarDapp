...
contract StarNotary is ERC721 {
    ...
    function buyStar(uint256 _tokenId) public payable {
        require(
            ...
            "This star is not currently up for sale."
        );
        uint256 starCost = starsForSale[_tokenId];
        address ownerAddress = ownerOf(_tokenId);
        require(msg.value > starCost, "You are not paying enough");
    ...
    function lookUptokenIdToStarInfo(uint256 _tokenId)
        public
        view
        returns (string memory starInfo)
    {
        starInfo = tokenIdToStarInfo[_tokenId].name;
